...
class Customer(models.Model):
    ...
    def show_status(self):
        ...
        ret = f'<span class="btn" style="color: white; background-color: {status_dic[self.status]};padding: 5px">{self.get_status_display()}</span>'
        return mark_safe(ret)

    def __str__(self):
        return f'{self.qq}-{self.name}'
...
class ConsultRecord(models.Model):
    ...
    def __str__(self):
        return  f'{self.customer.name}-{self.get_status_display()}-{self.consultant.name}'
...
class CourseRecord(models.Model):
    ...
    def __str__(self):
        return f'{str(self.re_class)} - ({self.day_num})'
